...
The production deployment uses a NumPy implementation of the trained ResNet18 model.
The original PyTorch training checkpoint is converted offline into `model/resnet18_fused.npz`,
so Vercel does not need to install PyTorch. This keeps the serverless function well below
Vercel's Python function bundle limit.

The explanation image is an occlusion-sensitivity heatmap that highlights regions that
most affect the predicted class confidence.
